...
def search_by_company_cause(search=None):
    if search is None:
        return "Not Found"
    property = Property.query.with_entities(
        Property.id).filter(Property.name.in_(search)).all()
    property_id = []
    for id, in property:
    ...
    company = Company.query.with_entities(Company.id).filter(
        Company.id.in_(company_has_property_list)).all()
    company_id = []
    for id, in company:
        company_id.append(id)
...
def prepare_response(company_id=None):
    ...
    com_dict = {}
    final_Data = []
    print("Company_IDS ==>", company_id)
    company = Company.query.filter(Company.id.in_(company_id)).all()
...
def search_company_based_on_filters(search_company_name=None,
    ...
    if search_company_name != "":
        ...
        print(company)
        flag = False
    if len(search_company_category[0]) != 0:
    ...
    if len(search_company_causes) != 0:
        ...
        flag = False
    if len(search_company_category[0]) != 0 and len(search_company_causes) != 0:
        company_id = intersection(company_causes, company_category)
        flag = False

    if flag:
        company_id = get_all_companies_id()
